...
RESOLUTION = (42, 42)
#Max number of rewards to be stored
MAX_NO_REWARDS = 100
...
def build_models(input_shape, num_actions):
    ...
    model.add(Conv2D(16, strides=3, kernel_size=6,  activation= 'relu', input_shape=input_shape))
    model.add(Conv2D(16, strides=1, kernel_size=3,  activation= 'relu'))
    model.add(Flatten())
    ...
    # anything
    model.add(Dense(num_actions, bias_initializer='ones',kernel_constraint=max_norm(0), activation="relu"))
...
def main(args):
    ...
    episodic_rewards = deque(maxlen=MAX_NO_REWARDS) 
    ...
    while step_ctr < args.steps:
        ...
        while not terminal:
            ...
            # for later plotting
            #print(q_values)
            store_q_values.append(q_values)
            ...
            if args.limit_fps:
                sleep(1 / 35.0)
        x.append(step_ctr)
        y1.append(np.mean(losses))
        y2.append(np.mean(store_q_values))
        ...
        print("Episode Average rewards " + str(np.mean(episodic_rewards)))
        print("Episode Mean of Q values " + str(np.mean(store_q_values)))
